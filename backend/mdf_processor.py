            ...
            with MDF(file_path) as mdf:
                ...
                if len(mdf.groups) > 0:
                    ...
                    if hasattr(first_group, 'channel_group') and first_group.channel_group:
                        # 가장 긴 시간 채널 찾기
                        max_time = 0
                        for ch_index in range(first_group.channel_group.ch_nr):
                            try:
        ...
        try:
            with MDF(file_path) as mdf:
                ...
                for group_idx, group in enumerate(mdf.groups):
                    ...
                    # 그룹 내 모든 채널 처리
                    for ch_idx in range(group.channel_group.ch_nr):
                        try:
                            # 채널명 먼저 확인
                            channel = group.channels[ch_idx]
                            ch_name = channel.name.decode('utf-8') if hasattr(channel.name, 'decode') else str(channel.name)
